document.addEventListener("DOMContentLoaded", async () => {
  ...
  let fullEmail = ""; // Variable to store the full email
  ...
  await chrome.storage.sync.get("email", (data) => {
      if (data.email) {
          fullEmail = data.email; // Store the full email
  ...
  saveEmailButton.addEventListener("click", async () => {
      ...
      if (email) {
          fullEmail = email; // Store the full email
  ...
  editEmailButton.addEventListener("click", () => {
      emailInput.value = fullEmail; // Use the full email
      mainView.style.display = "none";
      emailView.style.display = "block";
  });
  ...
  async function sendToWebhook(webhookUrl) {
      const payload = {
          ...
          "creator": fullEmail, // Use the full email
      };
